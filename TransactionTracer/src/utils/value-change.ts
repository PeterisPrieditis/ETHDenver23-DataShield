...
const tryFetchTrace = async (txhash: string, data:any = undefined) : Promise<[TraceResponse, TraceMetadata]> => {
    ...
    if (data) {
        ...
        traceResponse = await doApiRequest<TraceResponse>(`/api/v1/trace/${chain}/${txhash}`);
    }
...
const getTransactionMetadata = async (provider: Provider, txhash: string, data:any=undefined): Promise<boolean> => {
...
const main =  async (txhash: string, data:any = undefined) => {
    chainConfig = await getChain(chain);
    if (typeof chainConfig === 'undefined') {
        return {error: `Cannot find chain: ${chain}.`};
    }
    
    [traceResult, traceMetadata] = await tryFetchTrace(txhash, data);
    ...
    tokenMetadata = await fetchTokenMetadata(tokenMetadata, provider, Array.from(allTokens), data);
